...
class WebGraph():
    ...
    def make_personalization_vector(self, query=None):
        ...
        if query is None:
            ...
            for i in range(n):
                ...
                if self.url_satisfies_query(url, query):
                    v[i]=1
    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        for i in range(max_iterations):
            # Apply power method iteration using sparse matrix multiplication
            xprev = x0
            a = (1-alpha) * v
            x0 = alpha* torch.sparse.mm(self.P.t(), xprev) + a #(alpha*x0.t()*a + (1-alpha))*v.t()
            x0 /= torch.norm(x0)
            # Output debug information
            residual = torch.norm(x0 - xprev)
            logging.debug(f'i={i} residual={residual}')

            # Stop when sufficient "accuracy" is reached
            if residual < epsilon:
                logging.info(f'Power method converged after {i} iterations')
                break

           # xprev = x0  # Update for the next iteration

        return x0.squeeze()  # Return a squeezed vector for the final pagerank
